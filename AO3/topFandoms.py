...
if len(sys.argv) < 3:
    sys.exit(
        "Usage: %s <num top fandoms> <include umbrella fandoms?> (e.g.: %s 50 True)"
...
for category in mediaCategories:
    ...
    for fi in fandominfo:
        matchObj = re.search(
            r"<a class=\"tag\".*>(.*)</a>.*\(([0-9]+)\)", str(fi), re.I | re.S
        )
        if matchObj:
...
for category in sorted(mediaCategories):
    ...
    for key, value in sorted(
        iter(catFandoms.items()), key=lambda k_v: (k_v[1], k_v[0]), reverse=True
    ):
...
for key, value in sorted(
    iter(topFandoms.items()), key=lambda k_v1: (k_v1[1], k_v1[0]), reverse=True
):
    print("%d, %s, %s" % (i, key, value))
    if i >= numFandoms:
        break
