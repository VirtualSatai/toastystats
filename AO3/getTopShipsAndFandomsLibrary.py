...
from toastyTools import (
    setupUrllib,
    getAO3SearchURL,
    getAO3TagStructureURL,
    getBiggestKeyByValue,
)
...
class Tag:
    ...
    qualifyingWorks = (
        -1
    )  # the number of works in this tag that meet all the include/exclude requirements
...
def FetchTop10ShipsAndFandoms(primaryTag, includeTags, excludeTags):
    ...
    return (
        primaryTagData.categories["relationship"]["top"],
        primaryTagData.categories["fandom"]["top"],
    )
...
def AddNumWorksAndTopFandom(tag, include, exclude, minSize):
    ...
    if tag.totalWorks > minSize:
        ...
        print(
            "Ship total works ("
            + str(tag.totalWorks)
            + ") didn't surpass threshold size: "
            + str(minSize)
        )
        tag.qualifyingWorks = (
            -1
        )  # we're not going to fetch this unless the tag qualifies; -1 signals we didn't bother

    tag.ratio = float(tag.qualifyingWorks) / float(tag.totalWorks)
    if DEBUG:
        print(
            "Qualifying works: "
            + str(tag.qualifyingWorks)
            + "/"
            + str(tag.totalWorks)
            + " ("
            + str(round(100 * tag.ratio))
            + "%)"
        )
    return tag
...
# Returns number of qualifying works (factoring in include/exclude restrictions)
def AddFandomAndFetchNewCandidateFandoms(
    fandomName, include, exclude, pastFandoms, pastShips, minShipSize, candidateFandoms
):
    ...
    if fandomName not in list(pastFandoms.keys()):
        ...
        while not belowSizeThreshold:
            ...
            for shipName in topS:
                ...
                ship = AddShip(
                    shipName, include, originalExcludeTags, pastShips, minShipSize
                )
...
def AddFandomDeprecated(
    fandomName, include, exclude, pastFandoms, pastShips, minShipSize, recurse
):
    print(".......................................Fandom deep dive: " + fandomName)
    if fandomName not in list(pastFandoms.keys()):
        # get the basic fandom stats
...
def writeShipsToFile(ships, minSize, filename, discards):
    ...
    with open(filename, "w") as csv_ships:
        ...
        swriter.writerow(
            [
                "Ship",
                "Num works meeting requirements",
                "Top fandom meeting requirements",
                "Total works",
                "Top fandom overall",
                "Ratio of works meeting requirements",
            ]
        )
        dwriter.writerow(
            [
        ...
        for key in sorted(
            ships, key=lambda name: ships[name].qualifyingWorks, reverse=True
        ):
            s = ships[key]
...
def writeFandomsToFile(fandoms, minSize, filename, discards):
    ...
    with open(filename, "w") as csv_fandoms:
        ...
        for key in sorted(
            fandoms, key=lambda name: fandoms[name].qualifyingWorks, reverse=True
        ):
            f = fandoms[key]
...
def getTopShipsAndFandoms(
    ...
    for i in range(MAX_DEPTH):
        ...
        if smallestShip > minShipSize:
            ...
            #            time.sleep(PAUSE_INTERVAL)
            ships, fandoms = FetchTop10ShipsAndFandoms(
                primaryTag, includeTags, excludeTags
            )
            if DEBUG:
                print("including " + str(len(includeTags)) + " tags: ")
                print(includeTags)
            ...
            if DEBUG:
                print(
                    "*** Number of ships above size threshold: "
                    + str(numShipsAboveThreshold)
                )
            if numShipsAboveThreshold > 0:
                ...
                while candidateFandoms:
                    ...
                    if DEBUG:
                        ...
                        print(
                            ...
                            + str(j)
                            + "; "
                            + str(len(candidateFandoms))
                            + " CANDIDATES REMAIN"
                        )
                    fandom = AddFandomAndFetchNewCandidateFandoms(
                        fandomName,
                        completeIncludeTags,
                        originalExcludeTags,
                        topFandoms,
                    ...
                    writeShipsToFile(topShips, minShipSize, shipOut, shipDiscard)
                    writeFandomsToFile(
                        topFandoms, minShipSize, fandomOut, fandomDiscard
                    )
                ...
                print(
                    "********** no ships in this big loop surpassed size threshold!  Ending main fn."
                )
                break
...
def getTopShipsAndFandomsDeprecated(
    primaryTag,
    includeTags,
    excludeTags,
    minShipSize,
    ...
    for i in range(MAX_DEPTH):
        ...
        if smallestShip > minShipSize:
            ...
            for shipName in ships:
                ...
                ship = AddShip(
                    shipName, completeIncludeTags, originalExcludeTags, topShips
                )
                #                if ship.qualifyingWorks >= minShipSize:
                if ship.totalWorks >= minShipSize:
                    numShipsAboveThreshold += 1
